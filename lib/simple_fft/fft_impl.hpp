...
{
    real_type mult = 1.0f / num_elements;
    int num_elements_signed = static_cast<int>(num_elements);
                   ...
                   const FFT_direction fft_direction, const char *& error_description)
{
    ...
    {
    case(FFT_FORWARD):
        local_pi = static_cast<real_type>(-M_PI);
        break;
    case(FFT_BACKWARD):
        local_pi = static_cast<real_type>(M_PI);
        break;
    ...
    for (size_t i = 1; i < num_elements; i <<= 1)
    {
        ...
        sine = std::sin(static_cast<real_type>(0.5) * delta);    // supplementary sin
        // multiplier for trigonometric recurrence
        mult = complex_type(static_cast<float>(-2.0) * sine * sine, sin(delta));
